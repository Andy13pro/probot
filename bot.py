...
def procesar_respuesta_generica(texto_respuesta, comando):
 ...
 # Aplicar el patrón de eliminación específico de 'ACTAS REGISTRADAS' solo para ciertos comandos
    ...
    # A partir de aquí, las expresiones regulares se aplican para todos los comandos

    # Ajuste para eliminar (número) [n/n] en cualquier formato
    texto_respuesta = re.sub(r'\(\s*\d+\s*\)\s*\[\s*\d+\s*/\s*\d+\s*\]', '', texto_respuesta)

    # Suprimir todo desde '(' hasta ']'
    texto_respuesta = re.sub(r'\(.*?\]\s*', '', texto_respuesta)

    # Suprimir cualquier número entre paréntesis
...
async def handle_command(event):
    ...
    command_args = ' '.join(message.split()[1:])  # Obtener los argumentos después del comando

    print(f"handle_command: Usuario={username}, Comando={command}")

    # Configuración de comandos traducidos
    if command == '/rhf':
        command = '/dnif'
        ...
        original_messages[event.message.id] = {
            ...
            'command': command,
            'send_only_images': True,  # Enviar solo imágenes
            'retries': 0
        }
    elif command == '/dni':
        command = '/dnif'
        ...
        # Configuración para otros comandos sin traducción
        original_messages[event.message.id] = {
            'original_chat_id': event.chat_id,
            'original_user_id': sender.id,
            'command': command,
    ...
    # Determinar a qué bot enviar el comando
    target_bot = None
...
comandos_excluidos = [ '/dnifd']  # Añade los comandos que deban enviar media junto con texto

# Lista de comandos que solo deben enviar texto, sin imágenes o medios
comandos_solo_texto = ['/sbs', '/comando2']  # Agrega aquí los comandos que solo enviarán texto
